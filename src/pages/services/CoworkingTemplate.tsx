...
import { getCoworkingContent } from "@/data/coworking-content";
...
interface Props {
  routePath: string;
  /** When true, render as a campaign landing page (LpHeader + alt canonical). */
  lp?: boolean;
  /** Canonical path override for /lp/* pages. */
  lpPath?: string;
}

const CoworkingTemplate = ({ routePath, lp = false, lpPath }: Props) => {
  ...
  const content = getCoworkingContent(location.slug);

  const isMahape = location.slug === "mahape";
  const heroImg = isMahape ? mhReception : pick(0);
  const featureImg = isMahape ? mhOpenDesks : pick(1);
  ...
  const faqItems = [
    {
      q: `What is the price of ${service.label.toLowerCase()} in ${location.name}?`,
      a: `Our ${service.label.toLowerCase()} in ${location.name} starts from ₹599/day for day passes and ₹6,999/month for monthly memberships. Pricing varies based on plan length, team size and add-ons. Tour the campus to get a custom quote.`,
    },
    {
      q: `What is included in the ${service.label.toLowerCase()} at ${location.name}?`,
      a: `Every membership includes ${service.inclusions.join(", ")}, plus access to a vibrant member community across YesssWorks Mumbai & Navi Mumbai.`,
    },
    {
      q: `Is the ${service.label.toLowerCase()} in ${location.name} suitable for my team?`,
      a: `Yes, our ${location.name} ${service.label.toLowerCase()} is ideal for ${service.ideal.join(", ")}. Our team will help you find the right plan based on team size and growth plans.`,
    },
    {
    ...
    {
      q: `Can I get a GST invoice for my ${service.label.toLowerCase()} in ${location.name}?`,
      a: `Absolutely. We provide proper GST invoices for every membership and add-on. We can also assist with GST registration and company incorporation at our ${location.name} address.`,
    },
    {
    ...
    {
      q: `Do you offer day passes at the ${location.name} campus?`,
      a: `Yes, day passes start at ₹599 and include a hot desk, Wi-Fi, pantry access and 1 hour of meeting room time. Walk in or book in advance.`,
    },
  ];
  ...
  const sameServiceOtherLocations = pageRoutes.filter((p) => p.service === route.service && p.location !== route.location).slice(0, 5);
  ...
  const quickFacts = [
    ...
    { icon: Calendar, label: "Flexible monthly plans" },
    { icon: ShieldCheck, label: "CCTV + biometric access" },
  ];

  // Stakeholder hierarchy: enterprise → remote teams → startups → consultants → freelancers
  const audiences = [
    ...
    { icon: Rocket, title: "Startups & Founders", text: `Plug-and-play offices in ${location.name} so you can focus on shipping, not facility ops.` },
    { icon: Users, title: "Consultants", text: `Client-ready meeting rooms and reception support that wow every visitor.` },
    { icon: Sparkles, title: "Solopreneurs & Freelancers", text: `Quiet desks, fast Wi-Fi and a community that helps you grow.` },
  ];
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <SEO
        ...
        canonical={lp && lpPath ? lpPath : route.path}
              ...
              <div className="absolute bottom-4 left-4 right-4 bg-card/95 backdrop-blur rounded-xl p-4 shadow-[var(--shadow-card)] border border-border">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xl font-extrabold text-foreground">₹599 <span className="text-xs font-medium text-muted-foreground">/ day</span></p>
                  </div>
      ...
      {/* Plans */}
      <InlinePlans service={route.service} location={location.name} locationSlug={route.location} />


      {/* Features, Why this space */}
        ...
        <div className="container grid lg:grid-cols-2 gap-10 items-stretch">
          ...
          <Reveal delay={120} className="mt-6 text-base text-muted-foreground leading-relaxed space-y-4">
            <p>
              ...
              we have engineered every detail of our {service.label.toLowerCase()}, from ergonomic furniture and fibre-grade internet to a calm,
            ...
            <p>
              ...
              daily commute and client meetings painless. Need to scale up next quarter? Move into a {" "}
              <Link to={`/private-cabin-in-${location.slug}`} className="text-primary font-semibold hover:underline">private cabin in {location.name}</Link>
              {" "}or a {" "}
              <Link to={location.slug === "mahape" || location.slug === "navi-mumbai" ? "/office-suites-in-mumbai" : `/office-suites-in-${location.slug}`} className="text-primary font-semibold hover:underline">custom office suite</Link>
              {" "}without the hassle of relocating. Need a flexible meeting space for client calls? Our {" "}
              <Link to={`/meeting-room-in-${location.slug}`} className="text-primary font-semibold hover:underline">meeting room</Link>
              {" "}plans have you covered.
            </p>
            <p>
              Every YesssWorks {location.name} membership comes with high-speed leased-line internet, power backup, unlimited tea and coffee, business-grade printing,
            ...
            <p>
              Our members include first-time founders, profitable bootstrapped businesses, fast-scaling Series-A startups, regional sales teams of
              global enterprises, design studios, content agencies, legal practices, edtech teams and more. The result is a daily flow of warm
              introductions, partnerships, hires and customers, all under one roof in {location.name}. Take a quick {" "}
              <Link to={`/gallery/${location.slug}`} className="text-primary font-semibold hover:underline">photo tour of YesssWorks {location.name}</Link>
              {" "}or {" "}
              <Link to="/about" className="text-primary font-semibold hover:underline">read our story</Link>.
            </p>
          </Reveal>
          </div>
          <Reveal variant="scale" className="rounded-2xl overflow-hidden border border-border shadow-[var(--shadow-card)] min-h-[320px] lg:min-h-full">
            <img src={featureImg} alt={`${service.label} workspace at YesssWorks ${location.name}`} loading="lazy" className="w-full h-full object-cover" />
          </Reveal>
        </div>
      </section>
      ...
      <section id="faq" className="py-12">
        <div className="container max-w-3xl">
          ...
          <Reveal className="mt-10 text-center">
            <p className="text-sm text-muted-foreground">Still have questions about {service.label.toLowerCase()} in {location.name}?</p>
